      ...
      <div className="flex items-center gap-4 px-4 py-3 border-b border-panel-border bg-panel/90 backdrop-blur shrink-0 flex-wrap gap-y-2">
          ...
          {summary?.top_destination ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase text-gray-400">
                {summary.top_destination.tier?.replaceAll('_', ' ')}
              </span>
              <span
        ...
        {summary?.candidates?.length > 0 && (
          <div className="flex flex-col min-w-0 max-w-xs">
            <span className="text-[10px] text-gray-500 uppercase font-semibold tracking-wide">Ranked Candidates</span>
            <span className="text-xs text-gray-300 truncate">
              {summary.candidates.length} candidates, {summary.insufficient_evidence ? 'insufficient evidence' : 'evidence available'}
            </span>
          </div>
        )}
        ...
        {isDemo && (
          <span className="text-[10px] text-gray-500 shrink-0">
            Static fixture · {data.scoring_status || 'offline_demo'} · {data.model_version || 'demo model'}
          </span>
        )}
