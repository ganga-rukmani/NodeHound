  ...
  const handleTrace = async (config) => {
    ...
    try {
      ...
      if (err.status === 501) {
        ...
        setError({
          type: 'network',
          message: 'The live trace provider is temporarily unavailable. Please try again in a moment or load the Demo Trace.',
        });
      } else if (err.message?.includes('fetch')) {
        setError({
          type: 'network',
          message: 'Could not connect to the NodeHound backend. Make sure the API server is running on port 8000.',
        });
      } else {
        setError({
          type: 'general',
