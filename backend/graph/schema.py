...
class AttributionTier(str, Enum):
    """Evidence tier for attribution, separate from ML risk scoring."""
    KNOWN_SANCTIONED = "known_sanctioned"
...
class AddressNode:
    ...
    source_type: Optional[str] = None             # sanctions, verified_exchange, community_tag, etc.
    label_confidence: Optional[float] = None      # reliability of the label source, 0-1
    label_timestamp: Optional[datetime] = None    # when the source recorded the label
    ...
    def neo4j_properties(self) -> dict:
        ...
        return {
            ...
            "label_timestamp": self.label_timestamp.isoformat() if self.label_timestamp else None,
...
class TransferEdge:
    ...
    evidence_type: str = "direct_observed"
    edge_confidence: Optional[float] = 1.0
